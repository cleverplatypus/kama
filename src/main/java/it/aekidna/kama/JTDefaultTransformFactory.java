...
public class JTDefaultTransformFactory implements IJTTransformFactory
{
	...
	public JTDefaultTransformFactory()
	{
		...
		aliases.put( "dummy", DummyTransform.class );
	...
	@Override
	public void addTransformAlias( String inName,
			Class<? extends IJTTransform> inClass )
	{
		if ( aliases.containsKey( inName ) )
			throw new RuntimeException( "Attempt to override alias '" + inName
					+ "'" );
		if ( !IJTTransform.class.isAssignableFrom( inClass )
				&& !IJTTransformSetupDelegate.class.isAssignableFrom( inClass ) )
			throw new RuntimeException(
					"Attempt to add an alias of class'"
							+ inClass.getCanonicalName()
							+ "' "
	...
	@Override
	public void addTransformDelegateAlias( String inName,
			Class<? extends IJTTransformSetupDelegate> inClass )
	{
		if ( aliases.containsKey( inName ) )
			throw new RuntimeException( "Attempt to override alias '" + inName
					+ "'" );
	...
	public IJTTransformationSetup getTransformByPath( String inPath )
	{
		...
		try
		{
			System.out.println( "Returning transformer from file '" + inPath
					+ "'" );
			node = (ObjectNode) mapper
					.readTree( JTDefaultTransformFactory.class.getClassLoader()
							.getResource( inPath ).openStream() );
		}
		catch ( Exception e )
		{
			e.printStackTrace();
			throw new RuntimeException( "cannot resolve transform by path '"
					+ inPath + "'" );
		}
	...
	public IJTTransformationSetup resolveTransform( ObjectNode inNode )
	{
		if ( !inNode.has( "!" ) )
			throw new RuntimeException(
					"transformation node doesn't have a qualifier" );

		final String qualifier = inNode.get( "!" ).asText();
		IJTTransformationSetup setup = null;
		Class<?> clazz = null;
		if ( qualifier.matches( "^[a-z]\\w*$" )
				&& aliases.containsKey( qualifier ) )
		{
		...
		if ( clazz != null )
		{
			try
			{
				if ( IJTTransform.class.isAssignableFrom( clazz ) )
				{
					...
					setup = new JTTransformationSetup( transform,
							transform.parseConfig( inNode ) );
				}
				else if ( IJTTransformSetupDelegate.class
						.isAssignableFrom( clazz ) )
				{
					setup = ( (IJTTransformSetupDelegate) clazz.newInstance() )
							.setupTransform( this, inNode );
				}
				else
				{
		...
		if ( setup == null )
		{
			throw new RuntimeException( "unknown transformation qualifier: "
					+ qualifier );
		}
...
class SubTransformSetupDelegate implements IJTTransformSetupDelegate
{
	...
	@Override
	public IJTTransformationSetup setupTransform(
			IJTTransformFactory inFactory, ObjectNode inNode )
	{
		ObjectMapper mapper = new ObjectMapper();
		...
		try
		{
			config = (ObjectNode) mapper.readTree( IJTTransform.class
					.getClassLoader().getResource( sourcePath ).openStream() );
